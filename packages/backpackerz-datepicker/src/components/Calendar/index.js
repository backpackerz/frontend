...
class Calendar extends PureComponent {
	...
	onDragSelectionEnd = (date) => {
		...
		if (
			...
			isSameDay(newRange.startDate, date)
		) {
			this.setState(
	...
	estimateMonthSize = (index, cache) => {
		...
		const { start, end } = getMonthDisplayRange(
			monthStep,
			this.dateOptions,
		);
		const isLongMonth =
	...
	render() {
		...
		return (
			...
			>
				...
				<div
					className={classnames(this.styles.calendarBox, {
						[this.styles.calendarBoxToggle]: isToggleCalendarBox,
						[`${this.styles.calendarBoxToggle}--visible`]:
							this.state.visibleCalendarBox,
					})}
				>
					...
					{scroll.enabled ? (
						<div>
							{isVertical &&
								this.renderWeekdays(this.dateOptions)}
							<div
								className={classnames(
										...
										return (
											<Month
												...
												style={
													isVertical
														? {
																height: this.estimateMonthSize(
																	index,
																),
														  }
														: {
																height: scrollArea.monthHeight,
																width: this.estimateMonthSize(
																	index,
																),
														  }
												}
...
Calendar.propTypes = {
	...
	isToggleCalendarBox: PropTypes.bool,
};
